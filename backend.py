...
#上海的后端程序
def backend_process_shanghai(pipe_conn):
    print("后端程序启动")
    ...
    try:
        while True:
            ...
            result = main_shanghai(data, pdf_file=None)
            print(f"处理结果: {result}")
...
def main_shanghai(params,pdf_file):
    """
    主函数，接收参数并进行处理。
    ...
    end_pattern = re.compile(r"分行业|主要产品|单位|生产量|销售量|库存量")

    # 抽取pdf表格,产品的类目一般不会超过12行
    pattern = pdf_load.extract_pdf_table2_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3, end_pattern,
                                                   强匹配最大行数=12, 弱匹配最大行数=13, 弱匹配偏移量=1, 备用偏移量=1)

    # 解析pdf表格:列表字典数据结构
    pdf_dic = pdf_load.analysis_pdf_table2_shanghai(pattern)

    # ——————————————————————————————————开始匹配————————————————————————————————————

    # 前面的六个分类筛选，逐个提取
    sql_filtered = sql_filtered_table2_shanghai1
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
    ...
    sql_filtered = sql_filtered_table2_shanghai2
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered = sql_filtered_table2_shanghai3
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered = sql_filtered_table2_shanghai4
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
    ...
    sql_filtered = sql_filtered_table2_shanghai5
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
    ...
    sql_filtered = sql_filtered_table2_shanghai6
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table2_shanghai(
    ...
    # 筛选符合条件的 SQL 数据
    # 对数据进行分块，按照标准名称分成六类；对每一类的数据进行单独处理才不会导致数据混杂
    sql_filtered_table3_shanghai1 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '销量' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table3_shanghai2 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '产量' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table3_shanghai3 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '库存量' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table3_shanghai4 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '销量' and item.get('指标单位') == '%')
        )
    ]
    sql_filtered_table3_shanghai5 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '产量' and item.get('指标单位') == '%')
        )
    ]
    sql_filtered_table3_shanghai6 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '库存量' and item.get('指标单位') == '%')
        )
    ...
    # 结束匹配的字段
    end_pattern = re.compile(r"分行业|成本构成项目|本期金额|本期|成本|比例(%)")

    # 抽取pdf表格,产销售的类目一般不会超过12行，强匹配默认为1，硬编码
    pattern = pdf_load.extract_pdf_table3_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3,
                                                   end_pattern,
                                                   强匹配最大行数=12, 弱匹配最大行数=12, 弱匹配偏移量=1, 备用偏移量=1)

    # 解析pdf表格:列表字典数据结构
    pdf_dic = pdf_load.analysis_pdf_table3_shanghai(pattern)
    # ——————————————————————————————————3.产销表 开始匹配————————————————————————————————————
    # 前面的六个分类筛选，逐个提取
    sql_filtered = sql_filtered_table3_shanghai1
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path)
    # 保存匹配数据到 Excel
    ...
    sql_filtered = sql_filtered_table3_shanghai2
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered = sql_filtered_table3_shanghai3
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered = sql_filtered_table3_shanghai4
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold,
        get_high=get_high, get_medium=get_medium, get_low=get_low,
    ...
    sql_filtered = sql_filtered_table3_shanghai5
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
    ...
    sql_filtered = sql_filtered_table3_shanghai6
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table3_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL, threshold=threshold, low_threshold=low_threshold, get_high=get_high,
    ...
    # 筛选符合条件的 SQL 数据
    # 对数据进行分块，按照标准名称分成六类；对每一类的数据进行单独处理才不会导致数据混杂
    sql_filtered_table4_shanghai1 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '成本' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table4_shanghai2 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '成本占比' and item.get('指标单位') == '%')
        )
    ]
    sql_filtered_table4_shanghai3 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '成本' and item.get('指标单位') == '%')
        )
    ...
    # 结束匹配的字段
    end_pattern = re.compile(r"前五名客户.*|序号|客户名称|销售额|科目|.*费用化研发投入.*|.*研发人员数量.*")

    # 抽取pdf表格,成本的类目一般不会超过22行，强匹配默认为1，硬编码
    pattern,end_index = pdf_load.extract_pdf_table4_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3,
                                                   end_pattern,
                                                   强匹配最大行数=22, 弱匹配最大行数=22, 弱匹配偏移量=1, 备用偏移量=1)

    # 解析pdf表格:列表字典数据结构
    pdf_dic = pdf_load.analysis_pdf_table4_shanghai(pattern)
    # ——————————————————————————————————4.成本分析表 开始匹配————————————————————————————————————
    # 注意：要注意小计、合计，这两个
    # 前面的六个分类筛选，逐个提取
    sql_filtered = sql_filtered_table4_shanghai1
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table4_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
    ...
    sql_filtered = sql_filtered_table4_shanghai2
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table4_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, threshold_double=threshold_double, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered = sql_filtered_table4_shanghai3
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table4_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, threshold_double=threshold_double, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    # —————————————————————————————————————————————上交所 5.前五大客户 5.公司前五名客户 ———————————————————————————————————————————————

    # 筛选符合条件的 SQL 数据
    # 对数据进行分块，按照标准名称分成六类；对每一类的数据进行单独处理才不会导致数据混杂
    sql_filtered_table5_shanghai1 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '销售收入' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table5_shanghai2 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
    ...
    start_pattern2 = [
        re.compile(r'^1$'),  # 第一个单元格：严格匹配 '1'
        re.compile(r'.*公司.*|第一名.*|单位1.*'),  # 这些格式是弱匹配格式，因为无法主动区分客户和供应商
        re.compile(r'^\d*\.\d*$'),  # 第三个单元格：匹配任何数字（包括小数）
        re.compile(r'^\d+\.\d*|^\d+\.\d*%$')  # 第四个单元格：匹配任何百分比格式的值
    ]
    # 定义第三组备选模式（最终备用方案）
    ...
    # 抽取pdf表格,成本的类目一般不会超过22行，强匹配默认为1，硬编码
    pattern = pdf_load.extract_pdf_table5_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3,
                                                   end_pattern,
                                                   强匹配最大行数=7, 弱匹配最大行数=7, 弱匹配偏移量=0, 备用偏移量=1)
    # 强匹配最大行数=6, 弱匹配最大行数=15, 弱匹配偏移量=10, 备用偏移量=1)

    #优化抽取速度，传递end_index 参数！！！！
    if pattern == "5.text_start_signal":
        pdf_dic = pdf_load.text_analysis_pdf_table5(pdf_path, start_page=end_index, end_page=end_index+10)
    else:
        # 解析pdf表格:列表字典数据结构
        pdf_dic = pdf_load.analysis_pdf_table5_shanghai(pattern)


    # ——————————————————————————————————5.前五大客户表 开始匹配————————————————————————————————————
    # 注意：要注意小计、合计，这两个
    sql_filtered = sql_filtered_table5_shanghai1
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table5_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path)
    # 保存匹配数据到 Excel
    ...
    sql_filtered = sql_filtered_table5_shanghai2
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table5_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    sql_filtered_table6_shanghai1 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
            (item.get('标准名称') == '采购金额' and item.get('指标单位') != '%')
        )
    ]
    sql_filtered_table6_shanghai2 = [
        item for item in sql_dic
        if item.get('截止日期') == JJRQ and (
    ...
    # 抽取pdf表格,成本的类目一般不会超过22行，强匹配默认为1，硬编码
    pattern = pdf_load.extract_pdf_table6_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3,
                                                   end_pattern,
                                                   强匹配最大行数=7, 弱匹配最大行数=15, 弱匹配偏移量=10, 备用偏移量=1)
    ...
    if pattern == "6.text_start_signal":
        ...
        # 解析pdf表格:列表字典数据结构
        pdf_dic = pdf_load.analysis_pdf_table6_shanghai(pattern)

    # ——————————————————————————————————6.前五大供应商表 开始匹配————————————————————————————————————
    # 注意：要注意小计、合计，这两个
    sql_filtered = sql_filtered_table6_shanghai1
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table6_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path)
    # 保存匹配数据到 Excel
    ...
    sql_filtered = sql_filtered_table6_shanghai2
    mix_dic, unmatched_pdf_names, unmatched_sql_names = match.match_table6_shanghai(
        sql_filtered, pdf_dic, JZRQ, XXFBRQ, XXLL,
        threshold=threshold, low_threshold=low_threshold, get_high=get_high, get_medium=get_medium, get_low=get_low,
        message_only_wrong=message_only_wrong, mapping_file_path=mapping_file_path
    ...
    # 结束匹配的字段
    end_pattern = re.compile(r"本年度|上年度|费用化|资本化")

#——————————————————————————————————————抽取pdf数据————————————————————————
    # 抽取pdf表格,产品的类目一般不会超过12行
    pattern = pdf_load.extract_pdf_table7_shanghai(pdf_data, start_pattern1, start_pattern2, start_pattern3, end_pattern,
                                                   强匹配最大行数=8, 弱匹配最大行数=8, 弱匹配偏移量=2, 备用偏移量=0)

    # 解析pdf表格:列表字典数据结构
    pdf_dic = pdf_load.analysis_pdf_table7_shanghai(pattern)
    # ——————————————————————————————————开始匹配————————————————————————————————————
